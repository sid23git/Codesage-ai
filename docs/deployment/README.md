...
> **Confirmed failure mode if the build succeeds but the deploy then times out waiting on the health check:** Render assigns its own `PORT` env var per service and routes health checks (and all traffic) to it — it is *not* necessarily `8000`. `backend/Dockerfile`'s `CMD` binds Uvicorn to `${PORT:-8000}` (shell form, so the shell substitutes it at container start) specifically so this works automatically; you do not need to set `PORT` yourself in the service's env vars — Render supplies it. Reproduced locally: a container started with `PORT=10000` injected only ever answered on 10000 before this fix (never on 8000, where Render's check was pointed), and correctly answers on whatever `PORT` is set to after it. If you ever see a Docker service on Render build cleanly but the deploy times out with no useful build error, suspect this class of bug first — check that the Dockerfile's `CMD`/`ENTRYPOINT` actually reads `$PORT` rather than a hardcoded value.
